...
//         const copyRandomBlack = randomBlack.map(row => [...row.slice()]);
//         const randomNumber: number[][] = GenNumber.take(copyRandomBlack);
...
//             if (idx < 0) return bk(0);
//             // console.log('idx', idx)
//             // console.table(arr)
//             if (idx === cellsEmpty.length) return true;
//             const randomOke = this.randomValueCell(arr, cellsEmpty[idx].row, cellsEmpty[idx].col);
//             const key = `row:${cellsEmpty[idx].row}_col:${cellsEmpty[idx].col}_${JSON.stringify(arr)}`;
//             if (hash.has(key)) return bk(idx - 1);
//             if (idx > 0) hash.set(key, true);
//             if (randomOke) {
//                 return bk(idx + 1);
//             } else {
//                 arr[cellsEmpty[idx].row][cellsEmpty[idx].col] = EMPTY;
//                 return bk(idx - 1);
//             }
//         }
//         bk(0);
...
//     const fileName = "assets/gen_board/pack/special/" + i + ".json";
//     const arr = [];

//     for (let j = 0; j < 5; j++) {
//         const { board, randomBlack } = GenBoard.gen(i);
//         arr.push({ board, solution: randomBlack });

//     }

//     const dataJson = JSON.stringify({ boards: arr });
